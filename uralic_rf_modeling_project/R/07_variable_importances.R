...
# Desired variable order (top to bottom on y-axis)
var_order <- c(
...
# Assuming your data is in `all_importances` with columns: Language, Variable, Importance
...
# Plot with Okabe-Ito friendly gradient, normalized colors
...
# Generate partial dependence plot for top variable
...
# Combine All Language Panels into a Single PDF
# Create a list to hold all combined plots
combined_plots <- list()

# Loop again to collect plots instead of saving each individually
for (lang in names(results_list)) {
  if (is.null(results_list[[lang]]$model)) next
...
# Combine all into one big plot and save as PDF
